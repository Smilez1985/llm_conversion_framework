...
import winshell  # Optional: Falls nicht da, nutzen wir VBScript Fallback für Shortcuts
from win32com.client import Dispatch # Alternative für Shortcuts
...
class InstallerGUI:
    ...
    def on_combo_change(self, event):
        # Wenn User einen Namen (Key) auswählt, setze den Pfad (Value)
        # (Vereinfacht: Combobox zeigt direkt Pfade an)
        pass
    ...
    def run_checks(self):
        ...
        if self.check_connectivity():
            ...
            net_ok = False # Wir lassen Installation zu, warnen aber bei Downloads
    ...
    def install_process(self):
        ...
        if not EXE_SOURCE.exists():
            ...
            try:
                # Rufe das Build-Script auf, falls EXE nicht existiert
        ...
        try:
            ...
            # Kopiere auch notwendige Configs/Ordner falls sie nicht im PyInstaller Bundle sind
            # (PyInstaller sollte sie haben, aber sicher ist sicher für externe Assets)
    ...
    def create_desktop_shortcut(self, target_path):
        """Erstellt Verknüpfung via VBScript (keine extra dependencies nötig)"""
    ...
    def pull_docker_images_robust(self):
        ...
        images = ["debian:bookworm-slim"] # Minimal set, Orchestrator pulls others

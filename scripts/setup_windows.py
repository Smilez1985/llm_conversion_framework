...
# Zentrale Config (Fix vorgegeben)
...
class InstallerGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(f"{APP_NAME} Installer v2.6")
    ...
    def _init_ui(self):
        ...
        # Paths
        grp_app = ttk.LabelFrame(main_frame, text="Application Path", padding="10")
        ...
        grp_data = ttk.LabelFrame(main_frame, text="Data Path (Hidden)", padding="10")
        ...
        self.btn_install = ttk.Button(btn_frame, text="Install", command=self._start_install, state='disabled')
    ...
    def _install_process(self, app_dir: Path, data_dir: Path):
        try:
            self.log(f"Installing to: {app_dir}")
    ...
    def _create_shortcut(self, app_dir: Path, data_dir: Path, location: str):
        try:
            ...
            folder = winshell.desktop() if location == "Desktop" else winshell.programs()

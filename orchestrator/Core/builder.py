...
class BuildEngine:
    ...
    def _validate_docker_environment(self):
        ...
        try:
            ...
            try:
                # SECURITY: Using list args instead of shell=True
    ...
    def _prepare_build_environment(self, config: BuildConfiguration, progress: BuildProgress):
        ...
        if modules_src.exists():
            ...
            for module_file in modules_src.glob("*.sh"):
                ...
                # Ensure executable
                # SECURITY: nosec tells Bandit this chmod is intentional/required for Docker execution
                os.chmod(dst_file, 0o755) # nosec
        ...
        with open(config_file, 'w') as f:
            json.dump(build_config_data, f, indent=2, default=str)
    ...
    def _generate_dockerfile_content(self, config: BuildConfiguration, target_config: Dict) -> str:
        ...
        if config.target_arch == TargetArch.RK3566:
            dockerfile_lines.extend([
                ...
                # FIX: Correct Toolchain Name generated by config_module.sh
    ...
    def _validate_dockerfile_hadolint(self, dockerfile_path: Path, progress: BuildProgress):
        ...
        """
        try:
            # SECURITY: Use list args instead of shell=True
            result = subprocess.run(
                ["hadolint", "--version"],
                capture_output=True,
                text=True,
    ...
    def _build_with_buildx(self, config: BuildConfiguration, build_dir: Path, image_tag: str, progress: BuildProgress) -> Image:
        ...
        # Build command - SECURITY: List args
        cmd = [
            "docker", "buildx", "build",
            "--platform", platform,
            "--tag", image_tag,
    ...
    def _execute_build_modules(self, config: BuildConfiguration, progress: BuildProgress, image: Image):
        ...
        if hasattr(self.framework_manager.config, 'source_repositories'):
            for k, v in self.framework_manager.config.source_repositories.items():
                ...
                env_key = f"{key.upper()}_REPO_OVERRIDE"
                # Check if it's a dict (secure source) or string
                if isinstance(v, dict):
                    if 'url' in v: environment[env_key] = v['url']
                else:
    ...
    def _extract_artifacts(self, config: BuildConfiguration, progress: BuildProgress):
        """Extract build artifacts from temporary directory to final output"""

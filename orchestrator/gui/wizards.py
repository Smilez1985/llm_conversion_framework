...
class ModuleCreationWizard(QWizard):
    ...
    def create_intro_page(self):
        ...
        layout.addWidget(QLabel(tr("wiz.intro.text")))
        ...
        self.btn_import_std.setToolTip("Parses the file using strict rules. Fast, deterministic, no AI required.")
        ...
        self.btn_import_ai.setToolTip("Uses LLM to analyze hardware capabilities, suggest optimizations and write scripts.")
...
"""
    ...
    def run_standard_import(self):
        ...
        try:
            ...
            # Priority Check for Accelerators
            if config.get("SUPPORTS_CUDA") == "ON": 
                ...
                if config.get("NPU_MODEL"):
                    name_hint = f"Rockchip_{config['NPU_MODEL']}"
                else:
                    name_hint = "Rockchip_NPU"
                
                if config.get("SUPPORTS_RKLLM") == "ON":
                    sdk = "RKLLM / RKNN" # Hybrid
            ...
            if sdk == "CUDA":
                ...
                self.rad_ubuntu.setChecked(True) # Intel often prefers Ubuntu
            else:
                self.rad_debian.setChecked(True) # Robust default for Rockchip/Raspberry
            ...
            # ARM Specifics
            if config.get("SUPPORTS_NEON") == "ON": 
                cmake_flags.append("-DGGML_NEON=ON")
    ...
    def _ai_worker(self, path):
        try:
            coder = DittoCoder(
                ...
                config_manager=self.wizard().parent().framework_manager.config if self.wizard() and self.wizard().parent() else None
            )
    ...
    def on_ai_finished(self, config):
        ...
        # Hidden Field for build logic
        if "quantization_logic" in config: 
            self.quant_logic.setText(config["quantization_logic"])

...
class ModuleGenerator:
    ...
    def generate_module(self, data: Dict[str, Any]) -> Path:
        ...
        ensure_directory(target_dir / "scripts") # Python scripts go here
        
        # 2. Generate/Copy Files
        ...
        # 3. Copy Helper Scripts (Python)
        # REAL COPY: No simulation. Takes files from template/scripts to target/scripts
    ...
    def _copy_scripts(self, target_dir: Path):
        ...
        if src_scripts.exists():
            for item in src_scripts.glob("*.py"):
                ...
                self.logger.debug(f"Copied helper script: {item.name}")

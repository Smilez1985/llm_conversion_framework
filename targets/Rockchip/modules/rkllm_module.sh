...
SCRIPT_DIR="/app/scripts" # Hier liegen die kopierten Python-Skripte
...
main() {
    ...
    # 1. Toolkit Check (SSOT Source)
    if [ ! -d "$RKLLM_DIR" ]; then
        log "Cloning RKLLM Toolkit..."
        # Fallback URL defined in script, but ideally injected via Env
    ...
    # Wir nutzen das dedizierte Python-Skript statt Inline-Code
    CONVERTER="/app/scripts/export_rkllm.py"
    
    if [ ! -f "$CONVERTER" ]; then
        die "Converter script missing at $CONVERTER. Check Dockerfile COPY instruction."
    fi

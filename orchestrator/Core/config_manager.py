...
class ConfigSchema:
    ...
    def validate_value(self, value: Any) -> Tuple[bool, List[str]]:
        ...
        if value is not None and not isinstance(value, self.field_type):
            try:
                ...
                elif self.field_type == bool: value = str(value).lower() in ('true', '1', 'yes')
    ...
    def _apply_validation_rule(self, value: Any, rule: str) -> bool:
        try:
            if rule.startswith("min:"): return value >= float(rule.split(":", 1)[1])
            elif rule.startswith("max:"): return value <= float(rule.split(":", 1)[1])
            elif rule.startswith("regex:"):
                import re
                return bool(re.match(rule.split(":", 1)[1], str(value)))
            elif rule == "positive": return value > 0
            elif rule == "non_empty": return bool(value and str(value).strip())
        except: return False
        return True
...
class ConfigManager:
    ...
    def _ensure_directories(self):
        dirs = [self.config_dir, self.config_dir / "environments", 
                self.config_dir / "templates", self.config_dir / "backups"]
    ...
    def _initialize_core_schemas(self):
        schemas = [
            ...
            ConfigSchema("log_level", str, False, "INFO", "Log level", ["regex:^(DEBUG|INFO|WARNING|ERROR|CRITICAL)$"]),
            ...
            ConfigSchema("api_host", str, False, "127.0.0.1", "API host") # Security Fix: Localhost default
        ]
    ...
    def _load_default_configuration(self):
        ...
        for name, schema in self.config_schemas.items():
            if schema.default_value is not None:
                with self._lock:
                    self.config_values[name] = ConfigValue(
                        key=name,
    ...
    def _convert_env_value(self, val: str, schema: Optional[ConfigSchema]) -> Any:
        ...
        try:
            if schema.field_type == bool: return val.lower() in ('true', '1', 'yes')

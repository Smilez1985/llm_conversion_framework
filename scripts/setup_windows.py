...
def _find_repo_root_at_runtime() -> Optional[Path]:
    ...
    # KORRIGIERT: Marker für Installation Root
...
def install_application(destination_path: Path, desktop_shortcut: bool, log_callback: Callable, progress_callback: Callable):
    ...
    # Benutzerdefiniertes Ignore-Muster, das 'dist' und 'build' ausschließt
            ...
            while retries < max_retries:
                if self._check_internet_status():
                    ...
                    try:
                        ...
                        # KORRIGIERT: Logfarbe zu "error" für Fehler
                        self.log(f"Pull von '{img}' fehlgeschlagen: {e.stderr.strip()}", "error") 
                        retries += 1
                        time.sleep(5)
                else:
                    # KORRIGIERT: Logfarbe zu "warning" für Warung
...
class InstallerWindow(tk.Tk):
    ...
    # KORRIGIERT: Mapping der semantischen Farben auf Tkinter/Hex-Farben
    COLOR_MAPPING = {
        "info": "#007bff",      # Blau
        "success": "#28a745",   # Grün
        "error": "#dc3545",     # Rot
    ...
    def _init_ui(self):
        ...
        # KORRIGIERT: Behebt den NameError und verwendet korrekten Parent
        ...
        # LOGIK: Standardpfad vorschlagen, der sich anpassen lässt (lokales Benutzerprofil/Programme)
    ...
    def update_log(self, message: str, color: str = None):
        ...
        def do_update():
            ...
            # Konfiguriere Tag nur, wenn nötig (behebt TclError, falls Farbe noch nicht definiert ist)
            if tag_name not in self.log_text.tag_names():
                self.log_text.tag_config(tag_name, foreground=tk_color)
            
            # Füge die Nachricht mit dem entsprechenden Tag ein
    ...
    def _browse_for_folder(self):
        ...
        if folder_selected:
            ...
            # Logik: Hänge den Standard-Suffix an, falls der Benutzer nur ein übergeordnetes Verzeichnis gewählt hat
    ...
    def _start_installation(self):
        ...
        # KORRIGIERT: update_progress wird nur mit percent und message aufgerufen
...
if __name__ == '__main__':
    # NEU: Try/Except-Block, um unsichtbare Abstürze abzufangen und die Ursache anzuzeigen
    try:
        app = InstallerWindow()
        app.mainloop()
    except Exception as e:
        # Import messagebox at the last moment to ensure it's loaded if Tkinter failed
